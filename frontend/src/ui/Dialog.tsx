...
interface DialogProps {
  ...
  size?: Size;
  title?: string;
  /** When true, pressing space also closes the dialog. Use for read-only info dialogs. */
...
export function Dialog({ open, onClose, size = 'md', title, closeOnSpace, children }: DialogProps) {
  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => {
      ...
      if (closeOnSpace && e.key === ' ' && !(e.target instanceof HTMLInputElement) && !(e.target instanceof HTMLTextAreaElement)) {
        e.preventDefault();
        onClose();
      }
    };
